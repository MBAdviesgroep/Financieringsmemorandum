...
// Verwijder een eventuele data-URL-prefix en alle whitespace; valideer base64-tekens.
  ...
  try {
    if (!process.env.OPENAI_API_KEY) {
      return res.status(500).json({
        error: 'OPENAI_API_KEY ontbreekt. Voeg deze environment variable toe in Vercel en redeploy.',
      });
    }

    const body = await readJsonBody(req);
    const { filename, memorandum_url, extra_urls, extra_url, notities } = body || {};
    const dataBase64 = normalizeBase64(body?.dataBase64);

    if (!dataBase64 && !memorandum_url) {
      return res.status(400).json({
        error: 'dataBase64 (PDF) ontbreekt en er is ook geen memorandum_url meegegeven. Stuur de PDF-inhoud als base64 of een geldige blob-URL mee.',
      });
    }
    ...
    // dataBase64 heeft voorrang; anders de blob-URL van het hoofddocument.
    const mainFile = dataBase64
      ? {
          type: 'input_file',
          filename: String(filename || 'memorandum.pdf').replace(/[^\w.\- ]+/g, '-'),
