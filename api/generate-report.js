...
- Cover/klantnaam en kredietnemer: bepaal de kredietnemer strikt volgens de bron. Noemt de bron een vastgoed-B.V. (eventueel i.o.) als beoogde kredietnemer/vastgoedhouder en de werkmaatschappij als huurder, gebruik dan NOOIT een formulering waarbij de werkmaatschappij als kredietnemer wordt weergegeven — vermeld beide met hun juiste rol, bijv. "[Vastgoed-B.V.] / [Werkmaatschappij] groep", en verwijs er verderop naar als "de vastgoed-B.V." resp. "de werkmaatschappij". Noem een andere groepsmaatschappij ALLEEN "mede-kredietnemer", "hoofdelijk schuldenaar", "hoofdkredietnemer" of "borgsteller" als dat exact zo (of ondubbelzinnig gelijkwaardig) uit de bron blijkt — gebruik deze termen nooit automatisch of als aanname. Is dit niet expliciet in de bron onderbouwd, gebruik dan een neutrale formulering in de trant van: "De hypothecaire financiering wordt aangevraagd voor aankoop van het bedrijfspand via de vastgoed-B.V. Betrokken groepsmaatschappijen worden meegenomen voor huurkasstroom, structuur en eventuele aanvullende zekerheden."
...
- De financieringssamenvatting (afsluiting van het laatste hoofdstuk) is 6 tot 8 regels (nooit korter, nooit één algemene zin) en benoemt kort: onderbouwing van de aanvraag, eigen inbreng, historische resultaten, (vastgoed)structuur, zekerheden, betaalcapaciteit, aandachtspunten en het vervolg richting de financier.
...
function enforceFinalChecklist(r, warnings) {
  ...
  /* Kredietnemer-rollen (mede-kredietnemer, hoofdelijk schuldenaar,
     hoofdkredietnemer, borgsteller) mogen nooit automatisch aangenomen zijn —
     alleen ter waarschuwing (niet automatisch verwijderd, dat zou echte
     bronbevestigde informatie kunnen wissen). */
  ...
  if (/mede-?kredietnemer|hoofdelijk schuldenaar|hoofdkredietnemer|borgsteller/i.test(kredietnemerRolTxt)) {
    warnings.push('Eindcontrole: een partij is aangeduid als mede-kredietnemer/hoofdelijk schuldenaar/hoofdkredietnemer/borgsteller; verifieer dat dit expliciet uit de bron blijkt en geen aanname is.');
  }
  ...
  if (!hasTxt(afsluitTekst) || String(afsluitTekst).trim().length < 40) {
    ...
    if (afsluitZinnen.length < 4) {
      warnings.push('Eindcontrole: de financieringssamenvatting telt minder dan de gevraagde 6 tot 8 regels/zinnen en lijkt op één algemene afsluitzin; vul aan met onderbouwing van de aanvraag, eigen inbreng, historische resultaten, structuur, zekerheden, betaalcapaciteit, aandachtspunten en het vervolg richting financiers.');
    }
  }
