...
- Voorwaarden voor verdere beoordeling & documentatie (één hoofdstuk, tevens het laatste hoofdstuk van het rapport): één compacte tabel van MAXIMAAL 7 rijen met de kolommen Onderdeel | Status | Belang voor financier (geen aparte kolommen Type/Prioriteit, geen lege toelichtingen met "—", en gebruik nooit "voorwaarde", "actiepunt", "op te vragen", "te controleren" en "vervolgvraag" door elkaar voor hetzelfde onderwerp — elk onderwerp komt precies één keer voor, met als status bijvoorbeeld "Te controleren", "Op te vragen", "Te vestigen", "Te verifiëren", "Te monitoren", "Te onderbouwen" of "Nader af te stemmen"). Gebruik voor de documentatie uitsluitend twee compacte blokken — géén andere koppen: "Ontvangen / gebruikt" (bronmemorandum, jaarrekening/aangifte, taxatiegegevens, prognose-/kasstroominformatie, objectinformatie en overige daadwerkelijk aangeleverde of expliciet in de bron opgenomen stukken) en "Nog te controleren / op te vragen" (ontbrekende documenten, definitieve overeenkomsten, bewijs eigen inbreng, zekerheidsstukken, actuele cijfers — alleen wat relevant is voor de financieringsbeoordeling). Nooit interne foutmeldingen of kwaliteitscontrole-taal in deze lijsten (dus nooit dingen als "bronnen en aanwendingen sluiten niet", "kerncijfers niet onderbouwd" of "bronregel staat verkeerd") — dat soort constateringen los je vóór oplevering zelf op in de brondata, ze verschijnen nooit in het externe rapport. Gebruik nooit een aparte kop "Bijlage", "Bijlage(n)" of "Contactgegevens" in dit hoofdstuk.
- Financieringssamenvatting: GEEN apart hoofdstuk en GEEN losse laatste pagina — plaats deze als afsluiting onderaan ditzelfde hoofdstuk (08, na de documentatieblokken), maximaal 6 tot 8 regels, geen hard oordeel (zie CONCLUSIEBELEID). Dit is de inhoudelijke afsluiting van het hele rapport: eindig niet abrupt en niet leeg. Kantoor/adviseur en rapportdatum staan al op de cover en in de paginafooter — herhaal deze niet als apart contactblok.
...
function enforceFinalChecklist(r, warnings) {
  ...
  if (r.metadata?.rapport_type !== 'compact_intake' && hasTxt(ms.kernboodschap) && woorden < 120) {
    warnings.push('Eindcontrole: managementsamenvatting (hoofdstuk 1) is korter dan verwacht voor een volwaardig rapport; controleer of alle kernonderdelen (onderneming, doel, structuur, investering, financiering, resultaten, prognose, betaalcapaciteit, zekerheden, aandachtspunten) zijn benoemd.');
  }
  ...
  if (/zekerheid dekt volledig|nog aan te vestigen|op te richten eerste hypotheekrecht/i.test(zekerTxt)) {
    warnings.push('Eindcontrole: zekerhedentekst bevat mogelijk een niet-toegestane formulering; controleer vóór export.');
  }
