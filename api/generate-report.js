     ...
     financiele_analyse). Dit is een signaal, geen harde fout. */
  {
    ...
    if (periodes.length >= 2) {
      ...
      if (onvolledig.length) {
        warnings.push(`Eindcontrole: financiële tabel mist mogelijk cijfers voor ${onvolledig.join(', ')} terwijl andere jaren meer kernposten tonen; controleer of deze cijfers wél in de bron staan vóórdat een streepje getoond wordt.`);
      }
    }
  }
